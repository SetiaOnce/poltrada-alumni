...
class LoginController extends Controller
{
  ...
  {
    $pegawai_token = 'y0cPOsRb7pxdK1Ui0XnV7jKr8xYm39v7U9kskjP5RsKe5aDisfWHxQlXhdu9auJZ';
    setcookie("pegawai_token", $pegawai_token, time() + 10 * 365 * 24 * 60 * 60, "/");
    $body = [
        'pegawai_token' => $_COOKIE["pegawai_token"],
        'pegawai_aplikasi' => SsoAplikasi::where('id', 3)->first()->access,
    ];
    ...
    if($validator->fails()){
		...
		try{  
            $body = [  
                ...
                'access_key' => SsoAplikasi::where('id', 3)->first()->access,
            ];
            $api_response = Http::withOptions(['verify'=>false])
